...
class InventoryController extends ApiController
{
    ...
    public function getSearchQuery($location = '', $project_name = '', $block = '', $lot = '', $min_price = 0, $max_price = 0, $min_lot = 0, $max_lot = 0, $min_floor = 0, $max_floor, $lot_type = '', $house_model = '', $zone = '', $phase = '', $developer = '')
    {
        ...
        $units = Unit::active()
        ->select(\DB::raw("ANY_VALUE(date) AS date, ANY_VALUE(block_lot) as block_lot, ANY_VALUE(zone) as zone, ANY_VALUE(phase) as phase, ANY_VALUE(lot_area) as lot_area, ANY_VALUE(floor_area) as floor_area, ANY_VALUE(house_model) as house_model, ANY_VALUE(lot_type) as lot_type"))
        ->groupBy('block_lot')
        ->get();
